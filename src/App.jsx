...
const Landing = ({ onEnter }) => {
  const PLANS = [
    {
      name: "Starter", price: "Free", period: "", accent: false,
      desc: "Try TrendShop with no commitment.",
      features: ["5 trend scans per day", "Demo product matching", "Storefront staging", "AI analysis per trend"],
    },
    {
      name: "Pro", price: "$39", period: "/mo", accent: true,
      desc: "Everything you need to scale fast.",
      features: ["Unlimited trend scans", "Live CJ product catalog", "One-click storefront push", "Priority AI analysis", "Saved profiles & keys"],
    },
    {
      name: "Scale", price: "$99", period: "/mo", accent: false,
      desc: "For serious sellers doing real volume.",
      features: ["Everything in Pro", "Multi-store support", "Trend alerts via email", "Competitor tracking", "White-label ready"],
    },
  ];
...
// LOGIN / PROFILE MODAL
// ─────────────────────────────────────────────────────────────────────────────
const ProfileModal = ({ onSave, existing }) => {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, backdropFilter:"blur(4px)" }}>
      <div style={{ background:C.bgSurface, border:`1px solid ${C.borderMd}`, borderRadius:C.rLg, padding:32, width:"100%", maxWidth:400, animation:"fadeIn .2s ease" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:24 }}>
          <div style={{ width:32, height:32, borderRadius:10, background:C.accent, display:"flex", alignItems:"center", justifyContent:"center" }}>
          ...
          <div>
            <p style={{ fontFamily:"var(--font-display)", fontWeight:700, fontSize:16, color:C.text1, margin:0 }}>TrendShop</p>
            <p style={{ fontSize:11, color:C.text2, margin:0 }}>{existing ? "Welcome back" : "Create your profile"}</p>
          </div>
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div>
            <label style={{ fontSize:11, color:C.text2, display:"block", marginBottom:6, fontFamily:"var(--font-mono)", textTransform:"uppercase", letterSpacing:"0.08em" }}>Name</label>
            <input value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&save()} placeholder="Your name" type="text" style={{ letterSpacing:"normal", fontFamily:"var(--font-body)" }}/>
          </div>
          <div>
            <label style={{ fontSize:11, color:C.text2, display:"block", marginBottom:6, fontFamily:"var(--font-mono)", textTransform:"uppercase", letterSpacing:"0.08em" }}>Email <span style={{ color:C.text3 }}>(optional)</span></label>
            <input value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&save()} placeholder="you@email.com" type="email" style={{ letterSpacing:"normal", fontFamily:"var(--font-body)" }}/>
          </div>
          {err && <p style={{ fontSize:12, color:C.red, margin:0 }}>{err}</p>}
          <Btn onClick={save} variant="primary" style={{ marginTop:8, padding:"10px", justifyContent:"center", width:"100%" }}>
            {existing ? "Save profile" : "Create profile & launch →"}
          </Btn>
          <p style={{ fontSize:11, color:C.text3, textAlign:"center", margin:0 }}>Your API keys are saved locally and never sent anywhere.</p>
        </div>
      </div>
    </div>
  );
...
const Sidebar = ({ tab, setTab, profile, storefront, isLive, onProfileClick }) => (
  <aside style={{ width:"var(--sidebar-w)", background:C.bgSurface, borderRight:`1px solid ${C.border}`, display:"flex", flexDirection:"column", height:"100vh", position:"sticky", top:0, flexShrink:0 }}>
    {/* Logo */}
    <div style={{ padding:"20px 16px 16px", borderBottom:`1px solid ${C.border}` }}>
      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
        <div style={{ width:26, height:26, borderRadius:8, background:C.accent, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#0b0d13" strokeWidth="2.5"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
        </div>
        <div>
          <p style={{ fontFamily:"var(--font-display)", fontWeight:700, fontSize:14, color:C.text1, margin:0, lineHeight:1 }}>TrendShop</p>
          <p style={{ fontSize:9, color:C.accent, margin:0, fontFamily:"var(--font-mono)", letterSpacing:"0.06em" }}>{isLive?"● LIVE":"● DEMO"}</p>
        </div>
      </div>
    </div>
      ...
      {NAV_ITEMS.map(item => {
        ...
        return (
          <button key={item.id} onClick={() => setTab(item.id)}
            style={{ display:"flex", alignItems:"center", gap:10, width:"100%", padding:"9px 10px", borderRadius:C.rSm, marginBottom:2, background:active?C.bgEl:"transparent", color:active?C.text1:C.text2, border:`1px solid ${active?C.borderMd:"transparent"}`, fontSize:13, cursor:"pointer", fontFamily:"var(--font-body)", position:"relative" }}>
            <span style={{ color:active?C.accent:C.text3 }}>{item.icon}</span>
            {item.label}
            {item.id==="storefront" && storefront.length>0 && (
              <span style={{ marginLeft:"auto", background:C.accentDim, color:C.accent, fontSize:10, fontWeight:600, padding:"1px 6px", borderRadius:10, fontFamily:"var(--font-mono)" }}>{storefront.length}</span>
            )}
          </button>
        );
      })}
    ...
    <div style={{ padding:"12px 8px", borderTop:`1px solid ${C.border}` }}>
      <button onClick={onProfileClick}
        style={{ display:"flex", alignItems:"center", gap:10, width:"100%", padding:"10px", borderRadius:C.rSm, background:C.bgEl, border:`1px solid ${C.border}`, cursor:"pointer" }}>
        <div style={{ width:28, height:28, borderRadius:"50%", background:C.accentDim, border:`1px solid ${C.borderAcc}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700, color:C.accent, flexShrink:0, fontFamily:"var(--font-display)" }}>
          {profile.name.charAt(0).toUpperCase()}
        </div>
        <div style={{ textAlign:"left", minWidth:0 }}>
          <p style={{ fontSize:12, fontWeight:600, color:C.text1, margin:0, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{profile.name}</p>
          <p style={{ fontSize:10, color:C.accent, margin:0, fontFamily:"var(--font-mono)" }}>{profile.plan || "Starter"}</p>
        </div>
      </button>
    </div>
  </aside>
...
export default function App() {
  ...
  const loadTrends = async (key) => {
    ...
    log("Scanning 22 product hashtags on TikTok...");
  ...
  if (view==="landing") {
    return (
      <>
        ...
        {showProfile && <ProfileModal onSave={saveProfile} existing={profile}/>}
      </>
    );
  }
  ...
  return (
    <div style={{ display:"flex", height:"100vh", background:C.bgBase, overflow:"hidden" }}>
      ...
      <Sidebar
        ...
        profile={profile||{name:"User",plan:"Starter"}}
