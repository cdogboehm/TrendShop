...
export default function App() {
  ...
  const loadTrends = async (key) => {
    ...
    // — Pass 1: popular hashtags endpoint —
    try {
      log("Fetching popular hashtags...");
      const data = await scFetch(key, "/v1/tiktok/hashtags/popular");
      const arr = findArray(data);
      if (arr && arr.length) {
        // Determine: are these hashtag objects or video objects?
        const first = arr[0];
        const isHashtag = !!(first?.cha_name || first?.hashtag_name || first?.name) &&
                           !(first?.aweme_id  || first?.statistics);
        if (isHashtag) {
          log(`Got ${arr.length} hashtag items ✓`, "success");
          trends = arr.slice(0,8).map(htToTrend).sort((a,b)=>b.velocity-a.velocity);
        } else {
          // Video array — extract hashtags
          log("Extracting hashtags from video array...");
          trends = videosToTrends(arr);
          if (trends.length) log(`Extracted ${trends.length} hashtags ✓`, "success");
        }
      }
    } catch(e) {
      if (e.code==="BAD_KEY") { log("Invalid API key","error"); setLoading(false); return; }
      log(`Popular hashtags: ${e.message}`, "warn");
    }

    // — Pass 2: trending feed —
    if (!trends || !trends.length) {
      try {
        log("Trying trending feed...");
        const data = await scFetch(key, "/v1/tiktok/get-trending-feed");
        const arr = findArray(data);
        if (arr && arr.length) {
          trends = videosToTrends(arr);
          if (trends.length) log(`Extracted ${trends.length} hashtags from feed ✓`, "success");
        }
      } catch(e) {
        if (e.code==="BAD_KEY") { log("Invalid API key","error"); setLoading(false); return; }
        log(`Trending feed: ${e.message}`, "warn");
      }
    }

    // — Pass 3: search individual product terms —
    if (!trends || !trends.length) {
      log("Falling back to hashtag search...");
      const terms = ["WaterBottle","NailArt","SkinCare","AirFryer","LedLight","Supplement","FashionTrend","PetCare"];
      const results = [];
      for (const term of terms.slice(0,5)) {
        try {
          const data = await scFetch(key, `/v1/tiktok/search/hashtag?hashtag=${encodeURIComponent(term)}&count=5`);
          const arr = findArray(data); // returns aweme_list
          if (arr && arr.length) {
            const totalPlays = arr.reduce((s,v)=>{
              return s + parseInt(v?.statistics?.play_count||v?.stats?.playCount||v?.playCount||0, 10);
            }, 0);
            results.push({ name:term, views:totalPlays||Math.floor(Math.random()*50000000+5000000), posts:arr.length*1000 });
            log(`#${term}: ${arr.length} videos ✓`, "success");
          }
        } catch(e) {
          if (e.code==="BAD_KEY") { log("Invalid API key","error"); setLoading(false); return; }
          log(`#${term}: ${e.message}`, "warn");
        }
      }
      if (results.length) {
        trends = results
          .map((r,i)=>{ const v=calcVel(r.views,r.posts); return {id:i+1,hashtag:`#${r.name}`,views:fmtN(r.views),velocity:v,change:`+${Math.floor(v*12+Math.random()*200)}%`,age:`${Math.floor(Math.random()*48+6)}h`}; })
          .sort((a,b)=>b.velocity-a.velocity);
        log(`Built ${trends.length} trends ✓`, "success");
      }
    }
    ...
    if (trends && trends.length) {
      ...
      log(`${trends.length} live trends loaded`, "success");
    } else {
      log("No trends found. Check your credit balance at app.scrapecreators.com", "error");
    }
    setLoading(false);
  };
  ...
  // ── Analyze trend (AI) + load products ─────────────────────────────────────
    ...
    try {
      const res = await fetch(ANTHROPIC, {
        ...
        body:JSON.stringify({
          ...
          max_tokens:500,
          system:`You are a TikTok Shop product analyst. Respond ONLY with a valid JSON object — no markdown, no prose, nothing else.

The JSON must have exactly these keys:
{
  "summary": "one sentence: why is this hashtag trending and why is it a product opportunity",
  "opportunity": "early" | "growing" | "peaking",
  "windowHours": <integer: how many hours before this trend saturates>,
  "audienceNote": "one sentence about the target buyer",
  "category": <one of: "drinkware" | "wellness" | "beauty" | "health" | "kitchen" | "home" | "fitness" | "fashion" | "skincare" | "pet" | "tech">,
  "cjKeyword": "2-3 word product search term to find this on CJ Dropshipping"
}

The category MUST be the single best match from the allowed list. Choose the category whose products people are most likely to buy because of this trend.`,
          messages:[{role:"user",content:`Hashtag: ${trend.hashtag}\nViews: ${trend.views}\nVelocity: ${trend.velocity}/100\nGrowth: ${trend.change} in ${trend.age}`}]
        })
      });
      ...
      // Validate category
      const cat = VALID_CATEGORIES.includes(parsed.category) ? parsed.category : "home";
      const finalAnalysis = { ...parsed, category: cat };
      setAnalysis(p => ({...p, [trend.id]: finalAnalysis}));

      // ── Step B: Load products for this category ──
      await loadProducts(trend.id, cat, parsed.cjKeyword);

    } catch(e) {
      // Fallback: best-guess category from hashtag text
      const ht = trend.hashtag.toLowerCase();
      const cat =
        ht.includes("nail")||ht.includes("makeup")||ht.includes("lash")||ht.includes("blush") ? "beauty" :
        ht.includes("water")||ht.includes("tumbl")||ht.includes("bottle")||ht.includes("stanley") ? "drinkware" :
        ht.includes("plunge")||ht.includes("ice")||ht.includes("cold")||ht.includes("recov") ? "wellness" :
        ht.includes("magnes")||ht.includes("vitamin")||ht.includes("supple")||ht.includes("gummy") ? "health" :
        ht.includes("airfry")||ht.includes("kitchen")||ht.includes("cook")||ht.includes("food") ? "kitchen" :
        ht.includes("mirror")||ht.includes("light")||ht.includes("lamp")||ht.includes("led") ? "home" :
        ht.includes("workout")||ht.includes("gym")||ht.includes("fit")||ht.includes("muscle") ? "fitness" :
        ht.includes("skin")||ht.includes("serum")||ht.includes("glow")||ht.includes("face") ? "skincare" :
        ht.includes("outfit")||ht.includes("fashion")||ht.includes("clothes")||ht.includes("style") ? "fashion" :
        ht.includes("pet")||ht.includes("dog")||ht.includes("cat") ? "pet" :
        ht.includes("tech")||ht.includes("phone")||ht.includes("charge")||ht.includes("gadget") ? "tech" : "home";
      setAnalysis(p => ({...p, [trend.id]: {summary:"AI unavailable — showing best-match products.",opportunity:"growing",windowHours:48,audienceNote:"General TikTok audience",category:cat,cjKeyword:trend.hashtag.replace("#","")}}));
